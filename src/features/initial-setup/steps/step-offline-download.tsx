...
import { Download, CheckCircle2, HardDrive, ArrowLeft } from 'lucide-react';
    ...
    return (
        <div className="space-y-6">
            <div className="flex items-center gap-4">
                ...
                <div>
                    <Typography.MainTitle>{t('Download Offline Model')}</Typography.MainTitle>
                    <Typography.Paragraph className="text-zinc-400">
                        {t('Download the Parakeet model for local transcription.')}
                    </Typography.Paragraph>
                </div>
            </div>

            <div className="p-6 rounded-lg border border-zinc-700 space-y-4">
                <div className="flex items-center justify-between">
                    ...
                    {isModelAvailable ? (
                        ...
                        <Page.PrimaryButton onClick={handleDownload} disabled={isDownloading}>
            ...
            <div className="flex justify-between pt-4">
                {!isModelAvailable && (
                    <Page.SecondaryButton onClick={handleSkip} disabled={isDownloading}>
                        {t('Skip for now')}
                    </Page.SecondaryButton>
                )}
                <div className="flex-1" />
                <Page.PrimaryButton onClick={handleContinue} disabled={!isModelAvailable}>
                    {t('Continue')}
                </Page.PrimaryButton>
            </div>
        </div>
