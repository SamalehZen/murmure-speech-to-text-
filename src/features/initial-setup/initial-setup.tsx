...
export const InitialSetup = ({ onComplete }: InitialSetupProps) => {
    ...
    const [, setSelectedMode] = useState<'cloud' | 'offline' | null>(null);
    ...
    return (
        <div className="fixed inset-0 bg-zinc-900 flex items-center justify-center p-8">
            <div className="max-w-2xl w-full">
                ...
                {step === 'cloud-config' && (
                    <StepCloudConfig onComplete={handleConfigComplete} onBack={() => setStep('mode-choice')} />
                )}
                {step === 'offline-download' && (
                    <StepOfflineDownload onComplete={handleConfigComplete} onBack={() => setStep('mode-choice')} />
                )}
                {step === 'complete' && (
